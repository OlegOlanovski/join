...
/** @type {string[]} Urgent task IDs or status */
let Todos_urgent = [];
/** @type {Date|null} Nearest urgent task date */
 ...
 * @returns {Promise<Object|Array|null>} Node data or null
 */
...
/**
 * Syncs tasks from Firebase to IndexedDB (if available) and returns tasks.
 ...
 * - Waits for IndexedDB ready
 * - Syncs tasks
 * - Updates greeting and task stats
 */
async function init() {
...
/**
 * Finds all urgent tasks and updates dashboard urgent task section with nearest date.
 */
function getUrgrentTodo() {
  ...
  if (nearestUrgentDate) {
    if (urgent_tasks_months) urgent_tasks_months.innerText = months[nearestUrgentDate.getMonth()];
    if (urgent_tasks_day) urgent_tasks_day.innerText = nearestUrgentDate.getDate();
    if (urgent_tasks_year) urgent_tasks_year.innerText = nearestUrgentDate.getFullYear();
  }
}
