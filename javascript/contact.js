...
function closeModal() {
  ...
  box.addEventListener("transitionend", removeModalNow, { once: true });
}
...
function createFromForm() {
  let n = getEl("contactName"), e = getEl("contactEmail"), p = getEl("contactPhone");
...
function saveEdit(editId) {
  ...
  let n = getEl("contactName"), e = getEl("contactEmail"), p = getEl("contactPhone");
  let name = normalize(n ? n.value : "");
  let email = normalize(e/page? e.value : "").toLowerCase();
  let phone = normalize(p ? p.value : "");
  if (!name || !email) return;
...
function handleClick(e) {
  ...
  if (act && act.dataset.action && act.dataset.id) {
    ...
    if (a === "edit") return openModal("edit", contacts.find(function (x) { return x.id === id; }));
  }
